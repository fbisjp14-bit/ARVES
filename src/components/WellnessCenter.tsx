...
export function WellnessCenter({ externalData, onUpdate, apiKeys }: { externalData?: HealthData, onUpdate?: (data: HealthData) => void, apiKeys: { gemini: string } }) {
  const [data, setData] = useState<HealthData>(() => {
    ...
    const saved = localStorage.getItem('arves_health_data');
  ...
  useEffect(() => {
    ...
    window.addEventListener('arves_sync', handleSync);
    return () => window.removeEventListener('arves_sync', handleSync);
  }, [refreshData]);
