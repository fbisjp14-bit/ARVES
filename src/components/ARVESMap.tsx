...
interface ARVESMapProps {
  onClose: () => void;
  initialSearchQuery?: string;
  onLocationFound?: (placeName: string, coords: { lat: number; lng: number }) => void;
}

export const ARVESMap = ({ onClose, initialSearchQuery = '', onLocationFound }: ARVESMapProps) => {
  ...
  useEffect(() => {
    ...
    const currentProvider = (container as any)._arves_provider;
    ...
    if (currentProvider !== satProvider || !(container as any)._arves_tile_layer) {
      if ((container as any)._arves_tile_layer) {
        map.removeLayer((container as any)._arves_tile_layer);
      }
      const newTileLayer = L.tileLayer(tileUrl, { maxZoom: 19 });
      newTileLayer.addTo(map);
      (container as any)._arves_tile_layer = newTileLayer;
      (container as any)._arves_provider = satProvider;
    }
    ...
    if ((container as any)._arves_marker) {
      (container as any)._arves_marker.setLatLng([currentCoords.lat, currentCoords.lng]);
    } else {
      const marker = L.marker([currentCoords.lat, currentCoords.lng], { icon: customIcon }).addTo(map);
      (container as any)._arves_marker = marker;
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener('arves-navigate-map', handleNavEvent);
    return () => {
      window.removeEventListener('arves-navigate-map', handleNavEvent);
    };
  }, []);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-sm md:text-md font-medium tracking-tight text-white flex items-center gap-1.5">
              ...
              <span>Navegador Geográfico ARVES</span>
            </h2>
          </div>
        </div>
